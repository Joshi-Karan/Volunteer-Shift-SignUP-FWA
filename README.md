...
- **Integration**: a simulated email service (`src/emailService.js`) — logs a message rather than calling a real provider, to demonstrate the integration seam without adding an external dependency this week. It fires on two events: a successful signup (confirmation) and an admin deleting a shift that had signups (cancellation, once per affected volunteer).
- **Admin auth**: a session-token scheme, not a full auth system — see the "Admin Authentication" section below for the design and its known trade-offs.
